...
def validate_allele(row):
    assert row['Tumor_Seq_Allele2'] == row['Allele'], "Allele and Tumor_Seq_Allele2 do not match!"
...
def convert_maf_to_pdcm_format():
    ...
    vcf_to_pdcm_map = {"Hugo_Symbol": 'symbol', 'NCBI_Build':'genome_assembly', 'Chromosome':'chromosome','Start_Position':'seq_start_position','End_Position':'seq_end_position','Strand':'strand','Reference_Allele':'ref_allele', 'Allele':'alt_allele', 'Existing_variation':'variation_id', 't_alt_count': 'read_depth', 'Sequencer': 'instrument_model', 'Tumor_Sample_Barcode': 'sample_id'}
    ...
    for maf_file in glob("../mut/*maf"):
        ...
        mafDf.apply(validate_allele, axis=1)
        ...
        pdcm_table['platform_id'] = 'wxs_'  + '_' + pdcm_table['genome_assembly'] + '_' + pdcm_table['instrument_model']
        ...
        final_table = pdcm_table[["sample_id","symbol","read_depth","allele_frequency","chromosome","strand","seq_start_position","seq_end_position","ref_allele","alt_allele","variation_id","platform_id"]]
...
def build_biospecimen_uuid(cmp):
    builtTable = None
    uuid_file_path = "../resources/biospecimen_uuid.tsv"
