...
def generate_patient_sheet(hcmiDf, patient_template):
    ...
    align_and_append_sheet(patient_template, HCMI_patient_columns).to_csv("./resources/HCMI_metadata-patient.tsv", sep='\t', index=False)
...
def generate_sample_sheet(hcmiDf, sample_template, clinicalDf):
    ...
    align_and_append_sheet(sample_template, HCMI_sample_columns).to_csv("./resources/HCMI_metadata-patient_sample.tsv", sep='\t', index=False)
...
def generate_other_models_sheet(hcmiDf, other_model_template):
    ...
    align_and_append_sheet(other_model_template, HCMI_other_models_sheet)\
        .dropna(axis=1, how='all').to_csv("./resources/HCMI_metadata-other_models.tsv", sep='\t', index=False)
...
def generate_sharing(hcmiDf, sharing_template):
    ...
    align_and_append_sheet(sharing_template, HCMI_sharing_sheet).to_csv("./resources/HCMI_metadata-sharing.tsv", sep='\t', index=False)

hcmiDf = pd.read_csv("./resources/model-table.tsv", sep='\t')
clinicalDf = pd.read_csv("./resources/clinical.tsv", sep='\t')
patient_template = pd.read_csv("./resources/metadata_template-patient.tsv", sep='\t').dropna(axis=0, how='all')
sample_template = pd.read_csv("./resources/metadata_template-patient_sample.tsv", sep='\t').dropna(axis=0, how='all')
other_models_template = pd.read_csv("./resources/metadata_template-other_models.tsv", sep='\t').dropna(axis=0, how='all')
model_validation = pd.read_csv("./resources/metadata_template-model_validation.tsv", sep='\t').dropna(axis=0, how='all')
sharing_template = pd.read_csv("./resources/metadata_template-sharing.tsv", sep='\t').dropna(axis=0, how='all')
