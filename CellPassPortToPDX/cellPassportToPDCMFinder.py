...
def transformCellPassport_patient(cellPassportDf, patient_template):
    ...
    save_to_file = "HCMI_metadata-patient.tsv"
...
def transformCellPassport_sample(cellPassportDf, sample_template):
    ...
    save_to_file = "HCMI_metadata-sample.tsv"
    print(f'Saving {save_to_file}')
...
def transformCellPassport_other_models(cellPassportDf, other_models_template):
    ...
    cp_other_models_sheet['comments'] = cp_other_models_columns[["model_comments", "model_relations_comment"]]        .fillna('')        .agg(' '.join, axis=1)
    ...
    cp_other_models_sheet['external_ids'] = cp_other_models_columns[['prepended_cosmic_id', 'BROAD_ID', 'RRID', "prepended_CCLE_id"]]        .fillna('')        .astype(str)        .agg(','.join, axis=1)
    ...
    save_to_file = "CellModelPasport_other_models_metadata.tsv"
    print(f'Saving {save_to_file}')
...
def transformCellPassport_sharing(cellPassportDf, sharing_template):
    ...
    sharing_filename = "CellModelPasport_metadata-sharing.tsv"
...
def getTemplates():
    ...
    other_models_template = pd.read_csv("cell_and_organoid_metadata_template-other_models.tsv", sep='\t').dropna(axis=0,
                                                                                                                 how='all')
    ...
    return patient_template, sample_template, other_models_template, model_validation_template, sharing_template
...
def createCellPassportPdxFinderTemplates():
    ...
    patient_template, sample_template, other_models_template, model_validation_template, sharing_template = getTemplates()
    transformCellPassport_patient(filteredCpDf, patient_template)
    transformCellPassport_sample(filteredCpDf, sample_template)
    transformCellPassport_other_models(filteredCpDf, other_models_template)
