...
logging.basicConfig(filename='handle_field_column_updog.log', encoding='utf-8', level=logging.INFO, format='%(levelname)s:%(asctime)s: %(message)s', datefmt='%d/%m/%Y %I:%M %p')
start_dir = getcwd()
...
def handle_field_column(path):
    ...
    if len(tsv_files)>0:
        for f in tsv_files:
            metadata = pd.read_csv(join(path,f), sep='\t')
